...
class Config:
    ...
    def __init__(self, obstacle=None, goal=None):
        ...
        # 角速度边界d
        self.w_max = 70.0 * math.pi / 180.0  # [rad/s]
        self.w_min = -70.0 * math.pi / 180.0  # [rad/s]
        # 线加速度和角加速度最大值
        ...
        MAP_MAX_WIDTH = 10
        # 生成地图上边界
        for i in np.arange(-MAP_MAX_LENGTH // 2, MAP_MAX_LENGTH // 2,0.1):
            ox = i
            oy = MAP_MAX_LENGTH / 2.0
            self.ob = np.vstack((self.ob, [ox, oy]))

        # 生成地图右边界
        for i in np.arange(-MAP_MAX_WIDTH // 2, MAP_MAX_WIDTH // 2,0.1):
            ox = MAP_MAX_LENGTH / 2.0
            oy = i
            self.ob = np.vstack((self.ob, [ox, oy]))
        ...
        self.target = goal
        plt.plot(self.ob[:, 0], self.ob[:, 1], ".k")
        plt.show()
